...
#### v2.1
1.  エラー修正
- destinationが upである場合、目的地を間違えるエラーを修正。
2. リファクタリング
- コード構造改善 : 常数パッケージを`src/constants`内に移動

### v1.0
初期リリース

### v2.0 
1. 対象ウェブサイトのアップデートによる機能改善
- 状態メッセージと運休情報取得先が分離される
- ウェブサイトの要素クラスや全体の構造が変更される
2. リファクタリング
- コード構造改善 : parse および notification 内各パッケージを分け、モジュールの相互依存性を減らす
3. エラー修正 
- Notification　で DiscordManager　コールの際、間違ってパラメータを渡していたエラーを修正
4. 出力改善
- 出力詳細化: 🟢以前出発駅 🛑次到着駅 [通過済み -> 次通過（または到着予定）]
