...
서버 또는 디바이스가 필요합니다.
> 동작이 확인된 OS: `mac 15.6, 26.2` , `ubuntu 22.04`
...
#### v2.1
1. 오류 수정
- destination up인 경우, 목적지를 잘못 인식하는 오류 수정.
2. 리팩터링
- 코드 구조 개선 : 상수 패키지들을 `src/constants` 내로 이동

### v1.0
초기 릴리즈

### v2.0 
1. 웹 사이트의 업데이트로 인한 기능 개선
- 상태 메시지와 운행 정보 취득처가 분리됨
- 웹 사이트의 요소 클래스 등 전체적인 구조가 변경됨
2. 리팩토링
- 코드 구조 개선 : parse 및 notification 내　각 패키지 분리를 통한 모듈 상호 의존성 감소화
3. 오류수정 
- Notification의 DiscordManager호출 시, 잘못된 매개변수를 전달하던 오류 수정 
4. 출력 개선
- 출력 상세화: 🟢이전출발역 🛑다음도착역 [통과역 -> 다음통과역(또는 도착예정알림)]
